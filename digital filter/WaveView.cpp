...
CPoint cp;                 // создаем экземпляр класса Cpoint для того, чтобы получать положение  курсора на окне 
...
void CWaveView::OnDraw(CDC* pDC)
{
	...
	pDC->SetWindowExt(cp.x*20,128); // устанавливаем логические размеры окна. cp.x это координата по х положения курсора
	...
	HPEN hPen3=CreatePen(PS_SOLID,0,RGB(0,0,0)); //установка красного пера
	...
	for(int i=11;i<cp.x*20;i++)
	{
		...
		//	pDC->MoveTo(i,0);                            //рисование с помощью вертикальных линий
		...
		//pDC->LineTo(i,(buffer[i+105]-128)*cp.y/200); // рисование с помощью соединения точек
		Sleep(0);
	}
	


  HPEN hPen=CreatePen(PS_DOT,0,RGB(0,0,255)); //установка красного пера
...
pDC->MoveTo(2,0);
	...
	/*
	HPEN hPen2=CreatePen(PS_DASH,1,RGB(255,0,0)); //установка красного пера
